...
from gsv_tts.Download import download_file, unzip_file, validate_audio_url
...
def test_audio_url_policy_rejects_private_and_non_http_urls():
    with pytest.raises(ValueError):
        validate_audio_url("file:///etc/passwd")
    with pytest.raises(ValueError):
        validate_audio_url("http://127.0.0.1/audio.wav")


def test_zip_file_does_not_extract_sibling_prefix():
    with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
        root = Path(directory)
